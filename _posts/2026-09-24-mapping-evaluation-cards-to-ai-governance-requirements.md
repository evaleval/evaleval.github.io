...
This builds on a broader shift toward technical evaluation and assurance around the world. The UK's AI Safety Institute has been [conducting independent evaluations of advanced AI systems](https://www.gov.uk/government/publications/ai-safety-institute-approach-to-evaluations/ai-safety-institute-approach-to-evaluations) since 2023, including pre- and post-deployment assessments of potentially harmful capabilities. In the EU, the AI Act [requires providers of general-purpose AI models with systemic risk](https://artificialintelligenceact.eu/article/55/) to conduct and document state-of-the-art model evaluations, including adversarial testing; the Act also contemplates internal or independent external testing where appropriate, while the [GPAI Code of Practice](https://digital-strategy.ec.europa.eu/en/policies/contents-code-gpai) further develops expectations around pre-deployment evaluation, post-market monitoring, and the use of external evaluators. Similar efforts elsewhere, such as Singapore's [AI Verify](https://aiverifyfoundation.sg/what-is-ai-verify/), are pushing toward more structured approaches to testing, documentation, and independent scrutiny.
...
They can also reduce the burden on oversight bodies. When providers report evaluation evidence in different formats and at different levels of detail, regulators have to spend additional time locating, interpreting, and comparing the information they need. Standardized, structured reporting can make that process easier and also enable more reliable automatic parsing and analysis of evaluation evidence.
...
For the **60 pieces of evidence across 40 requirements whose natural home is evaluation or benchmark reporting**, EEE can already represent **56 of 60**. Fifty-one are captured through dedicated, typed fields.

For example, when California's SB 813 asks verification organizations to identify the model version they audited and when the audit took place, those map to EEE's model identifier (`model_info.id`) and evaluation timestamp (`evaluation_timestamp`). When the EU's GPAI Code of Practice asks providers to report the scaffolding, tools, and test-time compute behind an evaluation, EEE has typed fields for the tools available to an agent and for token limits. And whether an evaluation was run internally or by an external evaluator — something the AI Act, the Code of Practice, and the UK Parliament's Joint Committee on Human Rights all ask about — is recorded in a single field (`source_metadata.evaluator_relationship`).

The other five can currently only be noted in free-text fields. Which mitigations were active during an evaluation, for instance, fits only in EEE's catch-all model details. Adding [AutoBenchmarkCards](https://arxiv.org/abs/2512.09577) makes two additional pieces structured at the benchmark level. Both come from the AI Act's requirement to describe the data used for testing and validation: AutoBenchmarkCards records a benchmark's data type and languages as typed fields, where EEE can only note them in free text. The interactive visualization below shows how every piece of evidence maps.

The remaining four gaps are useful because they point to places where evaluation reporting can get better. All four come from the Safety and Security Chapter of the GPAI Code of Practice: representing evaluations that do not fit neatly into per-sample records, such as human-uplift studies; comparing results with and without mitigations; documenting random sample-selection methods; and recording why particular trajectories or additional samples were selected. EvalEval has active workstreams to extend EEE and Evaluation Cards to these and other kinds of evaluation.
