...
For the **60 pieces of evidence across 40 requirements whose natural home is evaluation or benchmark reporting**, EEE can already represent **56 of 60**. Fifty-one are captured through dedicated, typed fields. For example, when California's SB 813 asks verification organizations to identify the model version they audited and when the audit took place, those map to EEE's model identifier (`model_info.id`) and evaluation timestamp (`evaluation_timestamp`). When the EU's GPAI Code of Practice asks providers to report the scaffolding, tools, and test-time compute behind an evaluation, EEE has typed fields for the tools available to an agent and for token limits. And whether an evaluation was run internally or by an external evaluator, which the AI Act, the Code of Practice, and the UK Parliament's Joint Committee on Human Rights all ask about, is recorded in a single field (`source_metadata.evaluator_relationship`).

The other five can only be noted in free-text fields: which mitigations were active during an evaluation, for instance, fits only in EEE's catch-all model details. Adding [AutoBenchmarkCards](https://arxiv.org/abs/2512.09577) makes two additional pieces structured at the benchmark level. Both come from the AI Act's requirement to describe the data used for testing and validation: AutoBenchmarkCards records a benchmark's data type and its languages as typed fields, where EEE can only note them in free text. The interactive visualization below shows how every piece of evidence maps.

The remaining gaps are useful because they point to places where evaluation reporting can get better. All four come from the Safety and Security Chapter of the GPAI Code of Practice. They are representing evaluations that do not fit neatly into per-sample records, such as human-uplift studies; comparing results with and without mitigations; documenting random sample-selection methods; and recording why particular trajectories or additional samples were selected. EvalEval has active workstreams to extend EEE and Evaluation Cards to these and other kinds of evaluation.
